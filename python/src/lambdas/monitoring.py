...
def publish_message(loggroup, logstream, error_msg, lambda_func_name):
    sns_arn = os.environ['SNS_TOPIC_ARN']  # Getting the SNS Topic ARN passed in by the environment variables.
...
def handler(event, context):
    ...
    log_group, log_stream, error_message, lambda_func_name = get_error_details(payload)
    publish_message(log_group, log_stream, error_message, lambda_func_name)
